...
export const StudyProgress: React.FC = () => {
  ...
  const calculateOverallAverageScore = (cards: Flashcard[]): number => {
    const cardsWithScores = cards.filter(card => card.averageQualityScore > 0);
    if (cardsWithScores.length === 0) return 0;
    const totalScore = cardsWithScores.reduce((sum, card) => sum + card.averageQualityScore, 0);
  ...
  const calculatePerformanceByType = (cards: Flashcard[]) => {
    ...
    return types.map(type => {
      ...
      const avgScore = typeCards.length > 0
        ? Math.round((typeCards.reduce((sum, card) => sum + card.averageQualityScore, 0) / typeCards.length) * 10) / 10
        : 0;
  ...
  const calculatePerformanceByDifficulty = (cards: Flashcard[]) => {
    ...
    return difficulties.map(diff => {
      ...
      const avgScore = diffCards.length > 0
        ? Math.round((diffCards.reduce((sum, card) => sum + card.averageQualityScore, 0) / diffCards.length) * 10) / 10
        : 0;
