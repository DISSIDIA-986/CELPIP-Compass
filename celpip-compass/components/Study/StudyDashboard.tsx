...
export const StudyDashboard: React.FC = () => {
  ...
  const calculateStudyStreak = (cards: Flashcard[]): number => {
    const recentReviews = cards.filter(card => {
      ...
      const daysSinceReview = (Date.now() - card.lastReviewedAt.getTime()) / (1000 * 60 * 60 * 24);
  ...
  const updateStats = useCallback((cards: Flashcard[]) => {
    ...
    setStats({
      ...
      masteredCards: learningStats.mastered,
      learningCards: learningStats.learning,
      reviewCards: learningStats.review,
      newCards: learningStats.newCards,
      averageAccuracy: learningStats.avgAccuracy,
      studyStreak: calculateStudyStreak(cards)
    });
  }, []);

  const calculateNewAverageScore = (card: Flashcard, newQuality: number): number => {
    if (card.reviewCount === 0) return newQuality;
    const totalScore = (card.averageQualityScore * card.reviewCount) + newQuality;
    return Math.round((totalScore / (card.reviewCount + 1)) * 10) / 10;
  };

  const handleQualitySelect = (quality: number) => {
    if (!session.currentCard) return;

    const updatedCard = SpacedRepetitionService.calculateNextReview(session.currentCard, quality);
    const updatedFlashcard = {
      ...session.currentCard,
      metadata: updatedCard,
      nextReviewAt: updatedCard.dueDate,
      reviewCount: session.currentCard.reviewCount + 1,
      averageQualityScore: calculateNewAverageScore(session.currentCard, quality),
      status: SpacedRepetitionService.updateCardStatus(session.currentCard, quality),
    ...
    DataService.updateCard(updatedFlashcard);
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              >
                ...
                <option value={CardType.WRITING_TASK1}>邮件写作</option>
                <option value={CardType.WRITING_TASK2}>观点论证</option>
                <option value={CardType.SPEAKING_TASK}>口语任务</option>
                <option value={CardType.LISTENING_KEYWORD}>听力关键词</option>
              </select>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="bg-blue-50 p-4 rounded-lg">
                  <div className="text-xl font-bold text-blue-600">
                    {Math.round(session.completedCards.reduce((sum, card) => sum + card.averageQualityScore, 0) / session.completedCards.length * 10) / 10}
                  </div>
                  <div className="text-sm text-blue-600">平均质量分</div>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg">
                  <div className="text-xl font-bold text-purple-600">
                    {Math.round((session.completedCards.filter(c => c.averageQualityScore >= 4).length / session.completedCards.length) * 100)}%
                  </div>
