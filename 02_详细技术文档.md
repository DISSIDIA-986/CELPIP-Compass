...
**目标受众:** Development Team, DevOps, QA  
...
MONGODB_URL=mongodb://localhost:27017/celpip_dev
...
services:
  ...
  backend:
    ...
    environment:
      ...
      - MONGODB_URL=mongodb://root:password@mongodb:27017/celpip_dev
    depends_on:
      - postgres
      - redis
      - mongodb
...
### 2.2 Next.js 14 项目初始化（推荐使用 App Router）
...
// src/components/providers.tsx - 全局 Providers
'use client';
...
#### 响应式导航组件（符合 Next.js 最佳实践）

```typescript
// src/components/layout/navigation.tsx - 使用 Next.js Link 组件
'use client';
...
```
基础URL: https://api.celpip-compass.com/api/v1
...
PUT    /users/settings          # 更新设置

学习计划:
GET    /learning-plans          # 获取当前计划
POST   /learning-plans/generate # 生成新计划
PUT    /learning-plans/:id      # 更新计划
GET    /learning-plans/:id/tasks # 获取周任务

资源库:
GET    /resources               # 列表 (分页/筛选)
GET    /resources/:id           # 详情
GET    /resources/search        # 搜索
GET    /resources/featured      # 热门资源

进度追踪:
POST   /progress                # 记录完成
GET    /progress                # 获取进度
GET    /progress/dashboard      # 仪表板数据

社区:
GET    /community/stories       # 成功故事列表
POST   /community/stories       # 发布故事
GET    /community/qa            # Q&A列表
POST   /community/qa            # 提问
```

### 3.2 Express服务器初始化（增强错误处理）
...
CREATE TABLE users (
  ...
  current_clb_level SMALLINT DEFAULT 7,
  target_clb_level SMALLINT DEFAULT 8,
...
CREATE TABLE resources (
  ...
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  -- 性能优化：复合索引
  INDEX idx_resource_search ON resources(type, skill_category, difficulty_level) WHERE NOT is_deleted,
  INDEX idx_resource_created ON resources(created_at DESC) WHERE NOT is_deleted,
  INDEX idx_resource_rating ON resources(rating DESC, view_count DESC) WHERE NOT is_deleted,
  INDEX idx_source ON resources(source) WHERE NOT is_deleted
);
...
CREATE TABLE resource_completions (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(user_id, resource_id), -- 防止重复完成
  INDEX idx_user_completion ON user_id (created_at DESC),
  INDEX idx_resource_completion ON resource_id (created_at DESC)
);
...
CREATE TABLE progress (
  ...
  status VARCHAR(50) NOT NULL, -- 'completed', 'in_progress', 'skipped', 'pending'
  ...
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  -- 性能优化：索引
  INDEX idx_user_progress ON user_id (created_at DESC),
  INDEX idx_resource_progress ON resource_id (status, created_at),
  INDEX idx_task_progress ON task_id (status),
  INDEX idx_user_resource ON user_id, resource_id
);
...
CREATE TABLE study_sessions (
  ...
  completion_status VARCHAR(20), -- 'completed', 'incomplete', 'paused'
  focus_score INTEGER CHECK (focus_score BETWEEN 0 AND 100),
  distractions_count INTEGER DEFAULT 0,
  notes TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_user_session ON user_id (session_start_time DESC),
  INDEX idx_resource_session ON resource_id (session_start_time)
);
  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        type: { enum: ['writing', 'speaking', 'listening', 'reading'] },
...
### 6.1 安全的 JWT Token 流程设计

```
1. 安全登录流程
   ├─ 前端发送 credentials
   ├─ 后端验证凭证
   ├─ 生成 JWT Access Token (15分钟)
   ├─ 生成 Refresh Token (7天) + ROTATION TOKEN
   ├─ 保存 Refresh Token Hash 到数据库 (HttpOnly Cookie)
   └─ 返回 Access Token 到内存存储
...
3. Token 刷新流程 (with Rotation)
   ├─ 发送 Refresh Token 到 /auth/refresh
   ├─ 后端验证: 存在性 + 有效性 + 未过期
   ├─ 生成新的 Access Token + 新的 Refresh Token
   ├─ 将旧 Refresh Token 加入黑名单
   ├─ 保存新 Refresh Token Hash
   └─ 返回新的 Token 对

4. 安全登出
   ├─ 将 Refresh Token 加入黑名单
   ├─ 清除前端存储的 Access Token
   ├─ 清除 Session 数据
   └─ 返回成功状态
```
...
#### **代码分割和懒加载**
```typescript
// src/app/learning/page.tsx - 懒加载学习模块
...
const SkillBadge: React.FC<SkillBadgeProps> = ({
  ...
  const getSkillInfo = () => {
    switch (skill) {
      case 'listening':
        return { color: 'bg-blue-100 text-blue-800', label: '听力' };
      case 'speaking':
        return { color: 'bg-green-100 text-green-800', label: '口语' };
      case 'reading':
        return { color: 'bg-purple-100 text-purple-800', label: '阅读' };
      case 'writing':
        return { color: 'bg-orange-100 text-orange-800', label: '写作' };
      default:
    ...
    >
      ...
      <CardContent className="pt-0">
        ...
        <div className="flex items-center justify-between text-xs text-gray-500 mb-4">
          <span className="flex items-center gap-1">
            <Clock className="w-3 h-3" /> {resource.duration || 0}分钟
          </span>
          {resource.rating && (
            <span className="flex items-center gap-1">
...
describe('ResourceCard Component', () => {
  ...
  test('displays difficulty level', () => {
    render(<ResourceCard resource={mockResource} />);
    expect(screen.getByText('难度 CLB 8')).toBeInTheDocument();
  });
...
- [ ] **响应式**
  - [ ] 测试所有设备 (iPhone SE / 8 / 12 / iPad / Desktop)
  - [ ] 测试所有浏览器 (Chrome / Safari / Firefox)
  - [ ] 验证触控操作
